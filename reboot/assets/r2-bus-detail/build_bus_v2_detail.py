...
def make_image_material(name, image_path, roughness=0.58):
    mat = bpy.data.materials.new(name)
    mat.use_nodes = True
...
def textured_panel(name, location, width, height, material, col, parent=None, rear=False):
    x, y, z = location
    ...
    # UVs are assigned by vertex identity, independent of face winding.
    mapping = {0:(0,0), 1:(1,0), 2:(1,1), 3:(0,1)}
    for loop in mesh.loops:
        uv.data[loop.index].uv = mapping[loop.vertex_index]
...
def text_mesh(name, text, location, rotation, max_width, material, col, parent=None, size=0.24):
...
def add_production_details(body, root, cols, mats):
    ...
    # Baked Malayalam route boards: readable in GLB with no runtime font dependency.
    textured_panel("ROUTE_BOARD_FRONT_TEXTURE", (0, FRONT_END_Y - 0.073, 2.91), 1.34, 0.205,
                   mats["route_board"], cols["BODY_EXT"], body, rear=False)

    # Wear follows contact and weather: local road spray, doorway scuffs, isolated repair panels.
    for side_x, side in ((-1.276, "L"), (1.276, "R")):
        for axle_y, axle in ((FRONT_AXLE_Y, "F"), (REAR_AXLE_Y, "R")):
            cube(f"SPRAY_PATCH_{side}_{axle}", (side_x, axle_y + 0.22, 0.64),
                 (0.012, 1.18, 0.075), mats["dust"], cols["BODY_EXT"], body, 0.001)
    cube("REPAIR_PATCH_R_MID", (1.278, 1.75, 1.01), (0.019, 1.08, 0.42), mats["red_faded"], cols["BODY_EXT"], body, 0.006)
    cube("REPAIR_PATCH_L_REAR", (-1.278, 3.72, 1.02), (0.019, 0.86, 0.34), mats["red_repair"], cols["BODY_EXT"], body, 0.006)
    cube("FRONT_APRON_FADE", (0, FRONT_END_Y - 0.114, 0.69), (1.42, 0.018, 0.22), mats["red_faded"], cols["BODY_EXT"], body, 0.006)
    for i,(y,z,w) in enumerate([(-4.25,.45,.72),(-4.12,.62,.82),(-3.99,.79,.92)], 1):
        cube(f"STEP_WEAR_{i}", (-0.82, y-0.01, z+0.074), (w*0.72, 0.19, 0.012), mats["step_wear"], cols["BODY_INT"], body, 0.002)
    cube("DOOR_SILL_SCuff", (-1.282, -4.10, 0.90), (0.022, 0.94, 0.10), mats["step_wear"], cols["BODY_EXT"], body, 0.004)
...
def build_bus():
    ...
    mats = {
        ...
        "route_text": make_material("MAT_ROUTE_TEXT", (0.93, 0.78, 0.31, 1), 0.58),
        "red_faded": make_material("MAT_BODY_RED_FADED", (0.31, 0.050, 0.042, 1), 0.86),
        "red_repair": make_material("MAT_BODY_RED_REPAIR", (0.40, 0.055, 0.043, 1), 0.70),
        "dust": make_material("MAT_ROAD_DUST", (0.205, 0.075, 0.050, 1), 0.97),
        "step_wear": make_material("MAT_STEP_WEAR", (0.11, 0.115, 0.11, 1), 0.92, 0.10),
    }
    mats["route_board"] = make_image_material("MAT_ROUTE_BOARD_THRISSUR", BASE / "textures" / "route_thrissur.png", 0.60)
