...
export function setVirtualSteer(value: number) {
  virtualSteer = Math.max(-1, Math.min(1, value))
}

export function releaseVirtualControls() {
  for (const key of Object.keys(virtualState) as VirtualControl[]) {
...
export function createKeyboardInput() {
  ...
  return {
    sample(): DriveInput {
      ...
      const left = pressed.has("KeyA") || pressed.has("ArrowLeft") || virtualState.steerLeft ? 1 : 0
      const right = pressed.has("KeyD") || pressed.has("ArrowRight") || virtualState.steerRight ? 1 : 0
      const keyboardSteer = right - left
      const result = {
        throttle,
